...
### Открытый вопрос: события приняты, но property их не показывает

**Состояние на 2026-07-26 17:00 UTC.** Поток создан в 16:22:53 UTC, тестовые хиты отправлены в ~16:57. Realtime API отдаёт ноль по восьми разным запросам, включая поминутный разрез `minutesAgo` за окно, внутри которого хиты гарантированно лежат.

**Отсеяно проверками, а не рассуждением:**
- конфигурация потока — один `WEB_DATA_STREAM`, `measurementId` совпадает с `tid` в запросах, дублей потоков и property нет
- consent mode / CMP — в живом HTML нет ни `consent`, ни `analytics_storage`, ни `denied`, ни одного известного CMP. Это была главная альтернативная гипотеза: при `analytics_storage: denied` коллектор отвечает 204, а события в отчёты не попадают — ровно наблюдаемый симптом
- дубли тегов — ровно один `gtag/js`, один `gtag('config')`, один обработчик, старого UA-сниппета нет
- блокировщики — тест шёл в headless Chromium без расширений
- admin-плоскость property жива: кастомные измерения `method`, `link_location`, `link_url` читаются через metadata Data API

**Остаток после отсева — прогрев нового потока.** GA4 документирует до 30 минут на первые данные, у только что созданного property на практике дольше. Это гипотеза по исключению, а не установленный факт.

**Как подтвердить:** повторный замер Realtime и отчёта за `today` через час-полтора. Если данные появятся ретроспективно — был прогрев.

**Блокер диагностики:** у DebugView нет API, он существует только в UI, а в property сейчас **нет ни одного человеческого аккаунта** — только два сервис-аккаунта. Поэтому выдача `tech@niftsy.io` роли Administrator из блокера линковки с GSC превращается ещё и в блокер диагностики, и делать её надо первым действием.

**Сознательно не делали:** тест через Measurement Protocol решил бы вопрос за минуту, но пишет синтетическое событие в чистое property и создаёт лишний секрет. Ожидание бесплатно и не пачкает baseline.

### Что осталось и от кого зависит

**Требует действий владельца:**
1. **DNS TXT на `iber.dev`** — ✅ первая запись добавлена 2026-07-26, резолвится на 1.1.1.1, 8.8.8.8, 9.9.9.9.
   `google-site-verification=ddFX9ll5X3TZOLZHJRxaH22xwyetUqt_wPnMH0K_cTw`
   Cloudflare zone `2048b3ade1d37edad0e97049ef901fea`, record `0f148281dc6fe661a730c2bec70aa5a7` (id нужен для отката).
   Зона оказалась доступна нашему токену — предположение про чужой Cloudflare-аккаунт не подтвердилось.

   **Нужна вторая TXT-запись** — токен личного аккаунта `tech@niftsy.io`. Причина в разделе «Почему одной TXT-записи мало» ниже.
